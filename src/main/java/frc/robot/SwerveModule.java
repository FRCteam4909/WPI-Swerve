...
public class SwerveModule {
  ...
  public SwerveModule(
    ...
    SmartDashboard.putNumber("Override State - Speed", 0);
    SmartDashboard.putNumber("Override State - Angle", 0);
  }
  ...
  public void setDesiredState(SwerveModuleState state) {
    ...
    //  state = SwerveModuleState.optimize(state, new Rotation2d(m_yawEncoder.get()));

    

    if (SmartDashboard.getBoolean("Override State", false)) {
      state.speedMetersPerSecond = SmartDashboard.getNumber("Override State - Speed", 0);
      state.angle = Rotation2d.fromDegrees(SmartDashboard.getNumber("Override State - Angle", 0));
    }

    SmartDashboard.putNumber("State - speed", state.speedMetersPerSecond);
    SmartDashboard.putNumber("State - Angle", state.angle.getDegrees());
    ...
    var desiredMotorSpeedsRPM = getMotorSpeedsRPM(state.speedMetersPerSecond*5000, pidCalculatedYawRPM);
  ...
  public MotorRPMs getMotorSpeedsRPM(double wheelRPM, double yawRPM) {
    ...
    SmartDashboard.putNumber("Actual A RPM", m_driveMotorA.getEncoder().getVelocity());
    SmartDashboard.putNumber("Actual B RPM", m_driveMotorB.getEncoder().getVelocity());
